	...
	public function user_register($post = array())
	{
		...
		else
		{
			...
			else
			{
				...
				$data['subject'] = 'Verify your UrusDuit account';
	...
	public function activate_user($token = NULL)
	{
		if($token != NULL)
		{
			...
			if($rs == true)
			{
				...
				$db_account = array (
				'account_token'=>isset($token) && $token != '' ? trim($token) :'',
				'account_name'=>'Primary',
				'user_id'=>trim($user->id),
				'create_by'=>trim($user->id),
				'create_date'=>time(),
				'update_by'=>trim($user->id),
				'update_date'=>time()
				);
				
				$rs = $this->db->insert('user_account',$db_account);
